    ...
    const checkFavorite = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setIsFavorite(data.some((f: any) => f.id === id));
        }
      } catch (err) {}
    };
    checkFavorite();
    ...
    >
      <button 
        ...
        className={`absolute top-6 right-6 z-10 p-2 rounded-full border transition-all ${
          isFavorite ? "bg-red-50 border-red-100 text-red-500" : "bg-white/80 border-border text-foreground/20 hover:text-red-500 hover:border-red-100"
        }`}
      >
