...
const MobileNav = () => {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[100] bg-white border-t border-border px-4 py-3 pb-8 shadow-[0_-15px_40px_-20px_rgba(0,0,0,0.1)]">
          ...
          return (
            ...
            >
              <div className={cn(
                ...
                isActive ? "text-accent scale-110" : "text-primary/20 hover:text-primary/50"
              )}>
                {item.icon}
              </div>
              ...
              <span className={cn(
                ...
                isActive ? "text-accent" : "text-primary/20"
              )}>
                {item.label}
              </span>

              {isActive && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-accent rounded-full" />
              )}
            </Link>
          );
        })}
