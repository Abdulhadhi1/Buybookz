...
import { ShoppingCart, Star, Share2, ArrowLeft, Loader2, ShieldCheck, Truck, RefreshCw, Minus, Plus, Globe } from "lucide-react";
...
export default function BookDetailPage() {
  ...
  const addToCart = async (redirect = false) => {
    if (book.languages?.length > 0 && !selectedLanguage) {
        alert("Please select a language variation");
        return;
    }
          ...
          >
            <div className="space-y-6">
              <div className="flex flex-wrap items-center gap-4">
                <span className="px-5 py-2 bg-accent/10 text-accent text-[10px] font-black uppercase tracking-widest rounded-full border border-accent/20">
                  {book.category?.name || "General Selection"}
                </span>
            ...
            <div className="flex items-center space-x-12">
               <div className="flex flex-col">
                   <span className="text-[10px] uppercase font-black tracking-widest opacity-40 mb-1">Valuation</span>
               ...
               <div className="flex flex-col">
                   ...
                   <p className={`text-sm font-bold tracking-widest uppercase ${book.stock > 0 ? 'text-green-600' : 'text-red-600'}`}>
                       {book.stock > 0 ? `In Stock (${book.stock} Units)` : 'Out of Stock'}
                   </p>
               </div>
            </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                        {book.languages.map((lang: string) => (
                            <button 
                                ...
                                className={`px-8 py-4 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${selectedLanguage === lang ? 'bg-primary text-white shadow-xl scale-105' : 'bg-secondary text-primary/60 hover:bg-secondary/80 border border-border'}`}
                            >
                                {lang}
                            </button>
                        ))}
            ...
            <div className="space-y-8 pt-6">
                <div className="flex items-center space-x-6 text-[10px] font-black uppercase tracking-widest opacity-40">
                    <span className="italic">Order Quantity</span>
                ...
                <div className="flex flex-col sm:flex-row gap-5">
                    <button 
                      ...
                      className="flex-grow py-6 bg-primary text-primary-foreground rounded-full font-bold flex items-center justify-center space-x-4 shadow-2xl hover:bg-primary/95 transition-all active:scale-95 disabled:opacity-50 group"
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 p-8 bg-secondary/10 rounded-[2.5rem] border border-border items-center">
                ...
                <div className="flex flex-col items-center text-center space-y-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest opacity-60">Express Transit</span>
                </div>
                <div className="flex flex-col items-center text-center space-y-3">
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm">
                        <RefreshCw className="text-accent" size={24} />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest opacity-60">30-Day Policy</span>
                </div>
            </div>
          </motion.div>
        </div>
